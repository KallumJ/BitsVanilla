...
public final class PlayerApiUtils {

    private static GraphQLRPCClient rpcClient;

    private PlayerApiUtils() {
    }
     ...
     */
    public static @NotNull Optional<String> getUsernameForNickname(@NotNull String nickname) {
        JsonObject response;
        try {
    ...
    public static @NotNull CompletableFuture<Collection<String>> getNicknamesAsync() {
        try {
            return rpcClient.call(
                    ...
                    .thenApply(PlayerApiUtils::readEffectiveNames);
        } catch (IOException ex) {
            throw new RuntimeException("Error while getting player data", ex);
        }
    }
    ...
    public static CompletableFuture<Boolean> getHasTPDisabled(@NotNull ServerPlayerEntity player) {
        try {
            return rpcClient.call(
                    "query($uuid: ID!) { player(uuid: $uuid) { no_tp } }",
                    Map.of("uuid", player.getUuidAsString())
            ).thenApply(json -> json.getAsJsonObject("player").get("no_tp").getAsBoolean());
        } catch (IOException ex) {
            throw new RuntimeException("Error while getting player data", ex);
        }
    }
